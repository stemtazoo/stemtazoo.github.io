---
layout: page
title: 活性化関数とは？ReLU・Sigmoid・Softmaxの違い【G検定】
description: "活性化関数は、ニューラルネットワークに非線形性を与える関数です。中間層のReLU、二値分類のSigmoid、多クラス分類のSoftmaxを使う場所で切り分け、勾配消失やDead ReLUなどG検定で問われる特徴と欠点を整理します。"
...
ニューラルネットワークでは、各ノードで次のように、入力の重み付き和に活性化関数を適用します。
...
* しきい値で微分できず、それ以外でも勾配が0になるため、勾配を使う誤差逆伝播法での学習に不向き
