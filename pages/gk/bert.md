...
- **BERT（Bidirectional Encoder Representations from Transformers）**は、Transformerの**Encoder**を使い、文の前後を同時に考慮する双方向の自然言語処理モデルです。
- G検定では「**双方向の意味理解はBERT、一方向の生成はGPT**」と切り分けるのが最重要です。
...
一方、文章を左から右へ順に生成する用途ではGPT系が代表的です。BERTは「次の単語をどんどん生成するモデル」というより、**文の意味を理解して分類・抽出・質問応答に使うモデル**と考えると覚えやすいです。
...
| 一方向、Decoder、文章生成 | GPT |
| Attentionを使う基盤構造 | Transformer全般 |
| 単語の固定的な分散表現 | Word2Vecなど |
