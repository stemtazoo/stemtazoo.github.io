...
一方、収益のばらつきが大きいため、勾配推定の分散が大きくなりやすいという弱点があります。
...
* REINFORCE：基本形ではモンテカルロ収益を使う
* Actor-Critic：Criticが価値を推定し、Actorの更新を助ける

### ベースラインとの関係

REINFORCEでは、収益からベースラインを引くことで分散を小さくできます。

### 判断基準

* 「確率的方策を直接更新」→ REINFORCE
* 「モンテカルロ収益を使用」→ REINFORCE
* 「Q値を学習」→ Q学習・DQN
* 「ActorとCritic」→ Actor-Critic
