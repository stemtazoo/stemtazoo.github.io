...
判断するときは、**何を付加するか・何を計算するか** を見ると切り分けやすくなります。

| 問題文の合図 | 判断 |
|---|---|
| 生成多項式で割る・余りを付加 | CRC方式 |
| パリティビット・1の個数の偶数／奇数 | パリティチェック方式 |
| 水平方向・垂直方向のパリティ | パリティチェック方式 |
| 合計値・総和・補数 | チェックサム方式 |
| 1ビット誤りを訂正・誤り位置を特定 | ハミング符号方式 |

つまり、方式名を丸暗記するよりも、**検査用データをどう作るか** を見ます。
...
- パリティビットならパリティチェック、合計値ならチェックサム、1ビット訂正ならハミング符号と切り分ける
