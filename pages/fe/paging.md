...
このページでは、**ページ・ページフレーム・ページフォールトなど、ページング方式そのもの**を中心に扱います。オーバーレイ・ページング・スワッピングを横並びで見分ける場合は、[主記憶管理方式の違い](/fe/overlay-paging-swapping/) を参照してください。 固定区画・可変区画も含めた主記憶管理方式全体の比較は、[主記憶管理方式の違い](/fe/memory-management-methods/) で確認できます。
...
CPUが必要とするページが主記憶に存在しないと、**ページフォールト**が発生します。
...
ここで、置換え対象を決める方法として **FIFO** や **LRU** などのページ置換アルゴリズムが使われます。

- [FIFOページ置換とは？先に入ったページから追い出す考え方]({{ '/fe/fifo-page-replacement/' | relative_url }})
- [LRUページ置換とは？最も長く使われていないページを選ぶ方法]({{ '/fe/lru-page-replacement/' | relative_url }})
...
この状態は、[スラッシングとは？ページフォールトが多発して性能が低下する状態]({{ '/fe/thrashing/' | relative_url }})と関係します。
