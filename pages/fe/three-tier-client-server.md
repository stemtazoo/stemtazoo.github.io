...
FEでは、**「データを加工してプレゼンテーション層へ返す」ならファンクション層**と判断できるようにしておくと強いです。
...
ここでは、**DBへ問い合わせるための処理を担当する**と考えます。
...
この流れを押さえると、**どの層からどの層へ返すか**を問う選択肢も判断しやすくなります。
...
Webシステムでは、Webブラウザがプレゼンテーション層、Webサーバやアプリケーションサーバがファンクション層、DBMS・データベースがデータ層に対応します。
...
HTTPでは、クライアントが要求し、サーバが応答するのが基本です。また、HTMLはWebページの構造を記述するマークアップ言語であり、HTMLそのものを業務処理プログラムとしてサーバで実行するわけではありません。

業務処理をサーバ側に置くWebブラウザ型では、業務処理の変更のたびに専用アプリケーションを各クライアントへ配布・インストールする必要を減らせます。詳しくは[Webブラウザ型クライアントサーバとは？](/fe/web-client-server/)で整理しています。
...
ファンクション層はSQL文を組み立てたり、データベースアクセス層へ問い合わせ内容を渡したりすることがあります。

ただし、**SQLそのものを実行・解釈するのはDBMS側**です。
...
### HTTP要求はサーバからクライアントへ送る

逆です。基本的なWebの要求・応答では、ブラウザからWebサーバへHTTP要求を送り、WebサーバからブラウザへHTTP応答を返します。

### HTMLはサーバ側で動く業務プログラム

違います。HTMLはWebページの構造を記述するマークアップ言語です。サーバ側で実行する業務処理プログラムと混同しないようにします。
...
- データ加工はファンクション層
- DBへのアクセスはデータベースアクセス層
- SQLを実行・解釈するのはDBMS側
- プレゼンテーション層からDBへ直接アクセスしない
- 2層構成ではクライアント側が業務処理を持ちやすい
- HTTP要求はクライアント → サーバ、HTTP応答はサーバ → クライアント
