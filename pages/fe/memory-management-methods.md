...
このページは、**固定区画・可変区画・ページングなどの主記憶管理方式を横並びで見分ける比較記事**です。固定区画や可変区画の具体的な配置問題は、それぞれの個別記事で詳しく確認できます。 DRAM・SRAM・フラッシュメモリ・ROMのような「メモリそのものの種類」は、[メモリの種類](/fe/memory-types/) で別に整理しています。
