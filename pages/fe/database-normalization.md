...
このページでは、**表構造を第1〜第3正規形へ整理する設計**を中心に扱います。正規化した表をSQLで集計する基本は[SQLの集計関数](/fe/sql-aggregate-functions/)、更新処理を安全にまとめる考え方は[トランザクション](/fe/transaction/)で確認できます。
...
推移的関数従属を選択肢から見つける問題では、**右辺の属性を次の左辺としてつなげられるか**を見ると判断しやすくなります。
...
逆に、途中の矢印が1本でもなければ、推移的関数従属とは判断できません。
...
のように、つながらない矢印を無理に結び付けないことが重要です。

> **矢印は、問題文に示された関数従属だけを使ってつなぐ。**
...
### 推移的関数従属で、示されていない矢印を補ってしまう

選択肢にそれらしい項目が並んでいても、問題文に
...
の両方が示されていなければ、A → C を推移的に導くことはできません。
