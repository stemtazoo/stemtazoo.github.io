...
このページでは、**16進数1桁＝2進数4桁**を使った変換手順を中心に扱います。16進小数の各桁が `1/16`、`1/256` になる理由は、[16進小数とは？](/fe/hexadecimal-fraction/) で確認できます。 変換した2進数の小数点を動かして2倍・4倍・8倍する問題は、[16進小数を2のべき乗倍する方法](/fe/hexadecimal-fraction-bit-shift/) を参照してください。
...
16進小数 `0.C` を10進数へ変換します。

`C`は10進数の12であり、16進小数の小数第1位は `1/16` の位です。
...
このように、16進小数が1桁だけなら、数字の10進値を16で割るとすぐに求められます。
...
また、16進小数の文字を、そのまま10進小数の数字として読まないようにします。
