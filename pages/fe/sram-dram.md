...
description: SRAMとDRAMの違いを、フリップフロップ・コンデンサ・リフレッシュ・用途で整理し、SDRAM・RDRAMやEEPROM・フラッシュメモリとの切り分けまでFE科目A向けに解説します。
...
このページは、**SRAMとDRAMを試験で見分けるための比較記事**です。DRAM単体のリフレッシュや主記憶としての特徴は、[DRAMとは？](/fe/dram/) で詳しく確認できます。 SRAM単体のフリップフロップ・キャッシュ用途は、[SRAMとは？](/fe/sram/) で確認できます。

さらに、EEPROMやフラッシュメモリも「電荷」を利用して情報を保持しますが、こちらは**電源を切っても内容を保持できる不揮発性メモリ**です。
...
### EEPROM・フラッシュメモリとの切り分け

EEPROMやフラッシュメモリも、電荷を利用して情報を保持するメモリです。

ただし、DRAMとの大きな違いは**電源を切っても情報を保持できること**です。
...
そのため、問題文に「電荷」という言葉があっても、それだけでDRAMと判断しないことが重要です。

**コンデンサ・リフレッシュ・主記憶**ならDRAM系、**電源OFFでも保持**ならEEPROMやフラッシュメモリ、と切り分けます。
...
| クロック同期 | SDRAM |
| Rambus | RDRAM |
| 電源OFFでも保持 | EEPROM・フラッシュメモリ |
| 不揮発性 | EEPROM・フラッシュメモリ |
...
「電荷」という単語だけでは決めず、**揮発性か不揮発性か**まで確認するのがポイントです。
...
- RDRAMとSDRAMはDRAM系
- EEPROM・フラッシュメモリは不揮発性で、電源OFFでも保持する
- **「電荷」だけで決めず、リフレッシュが必要か・不揮発性かを見る**
- **フリップフロップ → SRAM、コンデンサ＋リフレッシュ → DRAM**
