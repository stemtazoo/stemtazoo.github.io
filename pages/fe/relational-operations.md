...
関係モデルでは、**属性（attribute）は列**を意味します。

そのため、問題文に「特定の属性だけを取り出す」とあれば、次のように読み替えるとすぐ判断できます。
...
逆に、「条件を満たす組（タプル）を取り出す」なら、組は行なので**選択（selection）**です。
...
この言い換えを先にすると、「選択」という日常語のイメージに引っ張られにくくなります。
