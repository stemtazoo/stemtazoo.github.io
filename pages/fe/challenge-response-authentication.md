...
NISTの用語集では、チャレンジレスポンスプロトコルを、**検証者がチャレンジ（通常はランダム値やnonce）を送り、認証される側が秘密情報と組み合わせてレスポンスを生成し、検証者がそのレスポンスを確認する認証プロトコル**として整理しています。

- [NIST CSRC：Challenge-Response Protocol](https://csrc.nist.gov/glossary/term/challenge_response_protocol)

FEでは具体的な暗号アルゴリズムより、次の流れを優先して押さえます。
...
NIST SP 800-63B-4では、nonceやchallengeによって認証処理の「新しさ」を確認する仕組みは、過去の認証メッセージをそのまま再送するリプレイ攻撃への耐性につながると説明されています。

- [NIST SP 800-63B-4：Authentication and Authenticator Management](https://pages.nist.gov/800-63-4/sp800-63b.html)
- [NIST CSRC：Nonce](https://csrc.nist.gov/glossary/term/nonce)
...
そのチャレンジに対応したレスポンス
↓
過去のレスポンスをそのまま再利用しにくい
```

と理解しておけば十分です。

なお、チャレンジレスポンスには共有秘密を使う方式だけでなく、秘密鍵を使う方式などもあります。**「必ずパスワードを使う方式」ではない**点にも注意します。
