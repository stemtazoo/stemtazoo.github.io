...
括弧が複数ある式や分数を含む式は、式全体を一気に変換しようとすると混乱しやすくなります。

FEでは、**普通に計算するときと同じ順番で、内側から1つずつ逆ポーランド表記へ変換する**と安定します。
...
の順に、**変換済みの部分を1つのかたまりとして扱う**のがポイントです。
...
### 括弧を消すときに、演算子の順番も消してしまう

逆ポーランド表記では、括弧そのものは書きません。

ただし、括弧が示していた**計算順序は、演算子を置く位置として残ります**。
...
迷ったら、括弧の中を先に1つのまとまりへ変換してから、そのまとまり同士を次の演算子でつなぎます。
...
- 複雑な式は、括弧の内側から順に1つずつ変換する
- 変換済みの部分は、次の演算では1つのかたまりとして扱う
