...
description: スタックのLIFO、push・pop、計算途中の値の一時保存、関数呼出し時の戻り番地やローカル変数の保存を、キューとの違いとあわせて整理します。
permalink: /fe/stack/
...
- スタックはLIFO（後入れ先出し）
- pushは追加、popは取出し
- 計算途中の値を一時保存し、最後に保存した値から取り出せる
- 関数呼出し時の戻り番地やローカル変数を保存する
- 最後に呼び出した関数から先に戻る
- キューはFIFO（先入れ先出し）
- 途中の値だけを変更したり、途中へ挿入したりするのは単純なスタックの基本操作ではない
