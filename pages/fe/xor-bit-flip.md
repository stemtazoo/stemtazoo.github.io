...
そのため、**残したい位置を1にしたマスクとANDをとる**と、必要なビットだけ取り出せます。
...
ANDの場合は、残したい位置を1にします。

例えば8ビット値の下位4ビットだけ残すなら、マスクは次のとおりです。
...
上位4ビットは0になり、下位4ビットは元の値がそのまま残ります。
...
ANDでは、**残したい位置を1、消したい位置を0**にしたマスクを使います。

8ビット値の下位4ビットだけ残したい場合は、
...
- ANDは必要なビットを取り出す
- ANDでは残したい位置を1にする
- `0F = 0000 1111` → 下位4ビットを残す
- `F0 = 1111 0000` → 上位4ビットを残す
- ORは必要なビットを1にする
- XORは必要なビットを反転する
