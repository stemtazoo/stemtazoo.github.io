...
```text
送信データを生成多項式で割った余りを付加し，n受信側で同じ生成多項式を使って誤りを検出する。
```
...
「奇数パリティ」「偶数パリティ」の奇数・偶数は、**検出できる誤りビット数**ではなく、正常なビット列に含まれる `1` の個数を表します。
...
試験では、**1方向だけなら基本は検出、行と列の2方向なら位置を特定して訂正できる**と切り分けます。
