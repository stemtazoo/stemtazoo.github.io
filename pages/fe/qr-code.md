...
倉庫の商品ラベルに付けられたQRコードが90度回転して貼られていた。それでも読み取り装置がコードの向きを判断しやすい主な理由として、最も適切なものはどれか。

- ア. 3隅に位置や向きを検出するためのパターンがある
- イ. QRコードは必ず上向きに印刷される規格になっている
- ウ. 格納された文字列の向きから回転角度を判断する
- エ. QRコード自体が回転角度を計算するプログラムを実行する
...
QRコードの3隅には位置検出パターンがあり、読み取り装置はこれを使ってコードの位置や向きを判断します。

QRコード自体がプログラムとして動作するわけではありません。

👉 判断ポイント  
**3隅の大きな四角 → 位置・向き・回転の認識**です。
