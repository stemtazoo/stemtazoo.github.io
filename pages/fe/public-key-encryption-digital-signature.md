...
このページは、**公開鍵暗号による暗号化とデジタル署名を試験で見分けるための比較記事**です。公開鍵暗号方式そのものの仕組みを詳しく確認したい場合は、[公開鍵暗号方式とは？](/fe/public-key-cryptography/) を参照してください。
...
ただし、デジタル署名で分かるのは**改ざんされたかどうか**です。
...
つまり、デジタル署名は**改ざん検知**には使えますが、**改ざん箇所の特定や訂正**を行う仕組みではありません。
...
| 署名を検証する | 送信者の公開鍵を使用 |
| 改ざんを検知する | デジタル署名 |
| 改ざん箇所を特定する | デジタル署名の役割ではない |
| 改ざんされた内容を元に戻す | デジタル署名の役割ではない |
| 他人の秘密鍵を送信者が使う | 不適切 |
...
デジタル署名の検証では、送信時と受信時の内容に違いがあることは確認できます。

しかし、**どの部分が書き換えられたかを特定する機能ではありません**。
...
また、デジタル署名だけで改ざんされた内容を元に戻すこともできません。

試験では、**「改ざんを検知できる」までは正しいが、「改ざん箇所を特定できる」「自動的に訂正できる」まで書かれていたら疑う**と切り分けます。

### 誤解6：公開鍵が本当に送信者のものかは自動的に分かる
...
- デジタル署名は改ざんの有無を検知できるが、改ざん箇所の特定や訂正はできない
- 他人の秘密鍵を使う選択肢は不適切
