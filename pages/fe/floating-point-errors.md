...
今回のように、**けた数に限度があり、最下位けたより小さい部分を四捨五入・切上げ・切捨てする** とあれば、丸め誤差を選びます。
...
例えば、8ビットの符号なし整数で表せる最大値は255です。

```text
255 + 1 = 256
```

しかし、8ビットに収まらないため、正しく表せないことがあります。
...
| 有限桁で表すために、切捨て・切上げ・四捨五入で生じる | 丸め誤差。正しい |
...
また、非常に大きい数と小さい数を足している場合は、情報落ちのような考え方が背景にあります。

```text
大きい数 + とても小さい数
→ 小さい数が結果に反映されにくいことがある
```
