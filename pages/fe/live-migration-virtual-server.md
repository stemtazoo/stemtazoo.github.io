...
FEで学ぶ知識・技能の範囲を確認するときは、IPAの「基本情報技術者試験（レベル2）シラバス」を一次情報として確認できます。現行版は **Ver.9.2** です。

- [IPA：試験要綱・シラバスについて](https://www.ipa.go.jp/shiken/syllabus/gaiyou.html)

### Microsoftの公式資料で見るライブマイグレーション

具体的な実装例として、Microsoft LearnではHyper-Vの**ライブマイグレーション**を、実行中の仮想マシンを停止時間をほとんど生じさせずに別の物理ホストへ移動する機能として説明しています。

- [Microsoft Learn：Hyper-V virtual machine live migration](https://learn.microsoft.com/en-us/windows-server/virtualization/hyper-v/manage/live-migration-overview)

ここで重要なのは、特定製品の操作方法を覚えることではありません。
...
という考え方が、FEで問われるライブマイグレーションの理解と対応します。

### 製品名と一般用語を混同しない

VMware製品では **vMotion** という名称がライブマイグレーション機能として知られています。一方、FEで問われる「ライブマイグレーション」は特定ベンダーの商品名ではなく、**稼働中の仮想マシンをホスト間で移動する一般的な技術概念**として理解するのが安全です。

試験対策では製品名を暗記するより、

> **稼働中の仮想サーバを、サービスをほぼ止めずに別の物理サーバへ移す**
