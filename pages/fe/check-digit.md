...
description: チェックディジットを、各桁に係数を掛けて合計し、modで余りを求める検査文字として整理します。通常の計算に加え、検査文字から未知の桁を逆算する方法や、パリティビット・CRC・ハッシュ値との違いも解説します。
...
チェックディジットと他の桁が分かっていて、元データの一部を求める問題もあります。

例として、次の規則を考えます。

```text
C = mod(N₁×1 + N₂×2 + N₃×3 + N₄×4, 10)
```
...
`40`は10の倍数なので、10で割った余りは`0`です。

したがって、余りを考えるときは取り除けます。
...
特に`mod 10`では、10の倍数を取り除き、1の位に注目すると速く判断できます。
...
> **各桁×係数を足し、最後にmodで余りを求める。逆算では割る数の倍数を取り除く。**
