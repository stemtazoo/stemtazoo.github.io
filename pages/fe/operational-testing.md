...
テスト工程は、**何を確認しているか**で切り分けます。

| テスト | 主に確認すること |
|---|---|
| 単体テスト | 個々のプログラムやモジュール |
| 結合テスト | モジュール間の連携やインタフェース |
| システムテスト | システム全体が要求・仕様を満たすか |
| 運用テスト | 実際の利用・運用で業務を行えるか |

試験中は、次のように考えると整理しやすくなります。
...
ここで大切なのは、運用テストでは単に「システム全体が動くか」だけでなく、**利用者や運用担当者の立場で本番運用に耐えられるか**を見ることです。
