...
このページは、**4つの用語を横並びで見分けるための比較記事**です。ARP・DNS・NATそのものの仕組みを詳しく知りたい場合は、それぞれの個別記事を参照してください。 IPアドレスとMACアドレスがルータ越しでどう使い分けられるかは、[IPアドレスとMACアドレスの違い](/fe/ip-mac-address-routing/) で確認できます。
...
ARPは、**IPアドレスからMACアドレスを調べる**仕組みです。

```text
IP → MAC
```

ARP要求・ARP応答・ARPキャッシュまで確認したい場合は、[ARPとは？](/fe/arp/) を参照してください。
...
DNSは、**ホスト名やドメイン名からIPアドレスを求める**仕組みです。

```text
名前 → IP
```

名前解決の流れを詳しく確認したい場合は、[DNSとは？](/fe/dns/) を参照してください。

### NAT

NATは、**IPアドレスを別のIPアドレスに変換する**仕組みです。

```text
IP → 別のIP
```

NATそのものやNAPTとの違いは、[NATとは？](/fe/nat/) と [NAPTとは？NATとの違い](/fe/nat-napt/) で詳しく確認できます。
