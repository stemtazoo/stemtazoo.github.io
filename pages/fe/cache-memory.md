...
このページでは、**CPUと主記憶の速度差を埋めるキャッシュメモリと、ライトスルー・ライトバック**を中心に扱います。DRAM・SRAM・フラッシュ・ROMを横並びで見分ける場合は、[メモリの種類](/fe/memory-types/) を参照してください。 ヒット率から平均アクセス時間を求める計算は、[キャッシュメモリのヒット率とは？](/fe/cache-hit-rate-average-access-time/) で整理しています。
...
### 判断軸4：キャッシュラインとページを混同しない

キャッシュミス時の転送は、通常**キャッシュライン（ブロック）単位**です。一方、仮想記憶では**ページ**という単位を使います。
...
したがって、「キャッシュの転送ブロックを仮想記憶のページと同じ大きさにすれば必ず効率が向上する」といった説明を、そのまま正しいとは判断しません。

### 判断軸5：レジスタと混同しない

レジスタもキャッシュも高速ですが、役割は異なります。
...
「高速アクセスできるからレジスタと同じ働き」という説明は誤りです。

### 判断軸6：TLB・MMU・主記憶と混同しない
...
このような**多段キャッシュ**は、CPUと主記憶の速度差が大きい場合に、実効アクセス時間を短くするために使われます。
...
### キャッシュミスでキャッシュ全体を消去するわけではない

キャッシュミスは、**必要なデータがキャッシュになかった**という意味です。
...
キャッシュ全体を一括消去して主記憶から全データを転送するわけではありません。
...
- 多段キャッシュは **実効アクセス時間の短縮**に役立つ
- キャッシュの転送単位は **キャッシュライン（ブロック）**
- 仮想記憶の単位は **ページ**
- キャッシュとレジスタは高速でも **役割が異なる**
- キャッシュミスで **キャッシュ全体を一括消去するわけではない**
