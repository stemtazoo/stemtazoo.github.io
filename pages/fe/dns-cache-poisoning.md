...
description: DNSキャッシュポイズニングを「DNSサーバのキャッシュに偽の名前解決情報を登録させる攻撃」として整理し、正しいURLでも偽サイトへ誘導される理由や、フィッシング・ファーミング・DNSSECとの違いをFE試験向けに解説します。
...
| DNS応答の正当性を電子署名で検証する | DNSSECによる対策 |

FE試験では、**DNSの名前解決結果が偽装される** と読めるかがポイントです。

DNSSECは、DNS応答に付けられた電子署名を検証し、応答が正当なものかを確かめる仕組みです。攻撃の名前ではなく、DNSキャッシュポイズニングなどへの対策として切り分けます。
