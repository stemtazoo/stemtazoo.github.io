...
また、FEではデータ構造でも「ハッシュ」という言葉が出ますが、目的が違います。

```text
情報セキュリティのハッシュ関数
→ 固定長の要約値を作り、改ざん検知などに使う

データ構造のハッシュ法・ハッシュ表
→ キーから格納位置を求める
```

データ構造側は、[ハッシュ法とは？衝突と一様分布の関係](/fe/hash-method-uniform-distribution/) と [ハッシュ表とは？](/fe/hash-table/) で整理しています。
