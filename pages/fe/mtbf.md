---
layout: page
title: MTBFとは？平均故障間隔を総稼働時間で考える【基本情報技術者試験】
description: MTBFを平均故障間隔として整理し、総稼働時間と故障回数から求める計算方法、MTTRとの違い、基本情報技術者試験での選択肢の切り方をやさしく解説します。
permalink: /fe/mtbf/
tags: [fe, fe-technology, computer-system]
fe_section: テクノロジ系
fe_subsection: システム構成要素
fe_order: 10
date: 2026-06-24
last_modified_at: 2026-09-20
---

## まず結論

MTBFとは、**故障してから次に故障するまでの平均時間**です。

```text
MTBF = 総稼働時間 ÷ 故障回数
```

複数台の機器をまとめて評価する場合は、1台分ではなく、全機器の稼働時間を合計します。

このページでは、**総稼働時間と故障回数からMTBFを求める計算**を中心に扱います。MTBF・MTTR・稼働率を横並びで整理したい場合は、[MTBF・MTTRとは？](/fe/mtbf-mttr/) を参照してください。

## 直感的な説明

MTBFは、「平均してどれくらい故障せずに動くか」を表す指標です。

```text
たくさん長く動いた
÷
故障した回数
```

という考え方です。

英語では Mean Time Between Failures といい、Between Failures が「故障と故障の間」を表します。

## 定義・仕組み

例えば、200台の機器を1日5時間、10日間使い、その間に2回故障したとします。

```text
総稼働時間 = 200台 × 5時間 × 10日
             = 10,000時間
```

```text
MTBF = 10,000時間 ÷ 2回
     = 5,000時間
```

ここで大切なのは、台数も含めて総稼働時間を求めることです。

### MTTR・稼働率との関係

| 用語 | 意味 | 良い方向 |
|---|---|---|
| MTBF | 平均故障間隔 | 長いほど良い |
| MTTR | 平均修理時間 | 短いほど良い |
| 稼働率 | 全時間のうち稼働できる割合 | 高いほど良い |

稼働率は、次の式で表されます。

```text
稼働率 = MTBF ÷ (MTBF + MTTR)
```

## 科目Aでどう出る？

計算問題では、次の順番で確認します。

```text
1. 台数を確認する
2. 1台あたりの稼働時間を求める
3. 全台数分の総稼働時間にする
4. 故障回数で割る
```

単位が「時間」になるかも確認すると、計算ミスを減らせます。

また、稼働率を高める条件も問われます。

```text
MTBFを長くする
→ 故障しにくくなる
→ 稼働率が上がる

MTTRを短くする
→ 修理が早くなる
→ 稼働率が上がる
```

### 保守施策とMTBF・MTTRを切り分ける

科目Aでは、保守方法がどちらの指標に効くかを問われることがあります。

| 施策 | 主に期待する効果 | 判断 |
|---|---|---|
| 予防保守 | 故障する前に点検・部品交換を行う | MTBFを長くする |
| 遠隔保守 | 現地へ行かずに障害対応しやすくする | MTTRを短くする |

覚える軸は次のとおりです。

```text
故障を起こしにくくする
→ MTBFを長くする
→ 予防保守など

故障後の復旧を早くする
→ MTTRを短くする
→ 遠隔保守など
```

したがって、**「遠隔保守でMTTRを長くする」「予防保守でMTTRを短くする」**のように効果の向きが逆なら、選択肢を切れます。

科目Aでは、**故障しにくさと修理の早さを別々に見る**ことが重要です。

## どんな場面で使う？

MTBFは、機器やシステムの信頼性を評価するときに使います。

例えば、次のような比較です。

- 同じ種類の装置で、どちらが故障しにくいか
- 改善前後で故障間隔がどれだけ伸びたか
- MTTRと組み合わせて、稼働率がどの程度になるか

ただし、MTBFが長くても、故障後の修理に長時間かかれば、稼働率が十分に高くならないことがあります。

```text
故障しにくさ
→ MTBF

修理の早さ
→ MTTR

両方を合わせた利用可能性
→ 稼働率
```

## よくある誤解・混同

### 1台分だけで計算する

複数台をまとめて評価する問題では誤りです。

```text
総稼働時間 = 台数 × 1台あたりの稼働時間
```

とします。

### MTBFとMTTRを混同する

| 用語 | 意味 | 良い方向 |
|---|---|---|
| MTBF | 平均故障間隔 | 長いほど良い |
| MTTR | 平均修理時間 | 短いほど良い |

### 予防保守と遠隔保守を同じ効果だと思う

違います。

```text
予防保守
→ 故障を防ぐ
→ MTBFを長くする

遠隔保守
→ 復旧を早める
→ MTTRを短くする
```

### MTBFが長ければ、稼働率は必ず十分高い

MTTRも影響するため、MTBFだけでは稼働率を決められません。

### 故障回数ではなく修理回数で割る

MTBFは故障と故障の間隔なので、問題文で示された故障回数を使います。

## まとめ（試験直前用）

- MTBFは平均故障間隔
- `総稼働時間 ÷ 故障回数`で求める
- 複数台なら台数も掛ける
- MTBFは長いほど故障しにくい
- MTTRは平均修理時間で、短いほど良い
- 予防保守は、故障を防いでMTBFを長くする方向
- 遠隔保守は、復旧を早めてMTTRを短くする方向
- 稼働率は `MTBF ÷ (MTBF + MTTR)`

{% include fe_article_footer.html %}