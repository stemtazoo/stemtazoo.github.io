...
現在の回線では、PPPoE以外の接続方式も使われますが、FE試験では **PPPoEは接続方式** として押さえるのが大切です。
...
科目Bでは、ネットワーク構成図や回線接続の説明でPPPoEが出ることがあります。
...
このとき、ルータがPPPoE接続を行う場合があります。

```text
ルータが接続ID・パスワードを使って接続
↓
インターネットへ通信できる
```

ただし、PPPoEがあれば複数端末が同時に1つのグローバルIPで通信できる、という意味ではありません。

複数端末の通信を外へ出す場面では、NAPTが関係します。
...
科目Bでは、装置名だけで判断せず、問題文が聞いている目的を読むのが大切です。

## よくある誤解・混同

PPPoEでよくある誤解は、**インターネット接続に関係する用語をすべて同じ役割だと思ってしまうこと** です。
