...
このページでは、**リピータ・ブリッジ・ルータに加えて、プロトコル変換を担うゲートウェイまで含めて比較**します。スイッチングハブを含め、OSI層・MAC・IPで整理したい場合は、[ルータ・リピータ・ブリッジ・スイッチングハブの違い](/fe/network-devices-router-repeater-bridge-switch/) を参照してください。
...
ルータはネットワーク層でIPアドレスを使って中継するため、**下位層の伝送媒体やアクセス制御方式が異なるネットワーク同士でも接続できます**。

試験では、次のような表現が組み合わされていれば、ルータを考えます。
...
「伝送媒体やアクセス制御方式が異なる」という言葉だけでゲートウェイと判断しないことがポイントです。**IPアドレスで経路を選ぶならルータ**、プロトコルやデータ形式そのものを変換するならゲートウェイです。

試験では、判断材料の**優先順位**まで決めておくと、ひっかけに強くなります。
...
プロトコルやデータ形式そのものを変換する
→ ゲートウェイ
```

つまり、選択肢に「伝送媒体が異なる」「アクセス制御方式が異なる」と書かれていても、後半に**IPアドレス**や**経路選択**があれば、そちらをより強い判断材料として見ます。
...
- ルータは下位層の伝送媒体やアクセス制御方式が異なるネットワークも接続できる
