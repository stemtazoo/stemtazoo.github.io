---
layout: page
title: ファンクションポイント法とは？機能の数と複雑さで開発規模を見積もる方法【基本情報技術者試験】
description: ファンクションポイント法を「外部入力・外部出力・内部論理ファイルなどの機能数と複雑さから開発規模を見積もる方法」として整理し、プログラムステップ法、標準タスク法、類推見積法との違いをFE試験向けに解説します。
...
基本情報技術者試験では、ファンクションポイント法は **外部入力、外部出力、内部論理ファイル、外部インタフェース、外部照会などから見積もる方法** として出題されやすいです。
...
今回の例題のように、**外部入出力や内部論理ファイルなどの個数や特性から開発規模を見積もる** なら、ファンクションポイント法です。
...
この説明では、**外部入力**、**内部論理ファイル**、**外部照会**、**外部インタフェース** が決め手です。

一方で、次のような説明はファンクションポイント法ではありません。
...
まず、各ファンクションタイプについて **個数 × 重み付け係数** を計算します。
その合計を出してから、最後に複雑さの補正係数を掛けます。
...
特に、補正前の `30` が選択肢にあると迷いやすいです。
問題文に複雑さの補正係数がある場合は、最後に掛け忘れていないかを確認します。
...
個数が0の行は計算結果も0になり、補正係数は各行に個別に掛けるのではなく、合計を出したあとに最後に掛けます。

## さらに注意したいひっかけ

ファンクションポイント法では、次のようなひっかけにも注意します。
...
特に、**ファンクションポイント法はステップ数を積算する方法ではない** という点が重要です。

ソースコードの行数ではなく、外部入力、外部出力、内部論理ファイルなどの機能に注目します。

また、ファンクションポイント法は、開発言語に依存しにくい見積方法として扱われます。
