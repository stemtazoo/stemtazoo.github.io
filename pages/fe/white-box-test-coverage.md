...
この「テストによってコードをどの程度実行・網羅できたか」を測る考え方が、**テストカバレージ分析**です。
...
網羅基準が増えると名前で混乱しやすいので、最初は **何を見ているか** を3段階に分けます。
...
特に、命令網羅・判定条件網羅・複数条件網羅は、次の図で位置を分けると整理しやすくなります。
...
見る場所を変えると、同じプログラムでも意味が違います。

| 網羅基準 | どこを見る？ | 一言で判断 |
|---|---|---|
| 命令網羅 | 分岐した先の処理 | 命令を全部通した？ |
| 判定条件網羅 | 判定式全体 | 真側・偽側を両方通った？ |
| 複数条件網羅 | 判定式の中の条件 | 真偽の組合せを全部試した？ |

ここでは、**複数条件網羅を「判定条件網羅のさらに強い版」とだけ覚えない**ことが大切です。  
まず「見る場所が違う」と分類すると、問題文を読みやすくなります。
...
の2ケースなら、AもBも「真・偽」を少なくとも1回ずつ取るため、条件網羅を満たします。

一方、**複数条件網羅**は、AとBの**組合せを見ます**。
...
2条件なら、真偽の組合せは4通りです。そのため、すべてを確認するには4ケース必要です。

試験では、次のように切り分けると迷いにくくなります。
...
**「各条件」なら条件網羅、「全組合せ」なら複数条件網羅**と判断するのがポイントです。
...
流れ図に複数の分岐がある場合は、**各判定を別々に確認してから、必要な網羅条件を重ね合わせる**と整理しやすくなります。
...
### テストカバレージ分析と静的コード解析は同じ？

違います。

テストカバレージ分析では、**テストを実行した結果、コードのどこまでを通ったか**を測ります。

一方、静的コード解析は、**プログラムを実行せずにソースコードを解析**します。
...
試験問題で「コード中のどれだけの割合を実行できたか」のような表現があれば、テストカバレージ分析を考えます。
...
条件網羅で確認するのは、**個々の条件がそれぞれ真・偽を取ったか**です。

2条件なら、例えば「真・真」と「偽・偽」の2ケースでも条件網羅を満たせます。

これに対して、**真・真、真・偽、偽・真、偽・偽のすべてを要求するのが複数条件網羅**です。
...
- テストカバレージ分析：テストでコードをどの程度実行・網羅できたかを測る
- 静的コード解析：プログラムを実行せずにコードを解析する
