...
このページは、**DRAM・SRAM・フラッシュメモリ・ROMを比較するメモリ種類の総論**です。CPUと主記憶の間に置くキャッシュの仕組みや書込み方式は、[キャッシュメモリとは？](/fe/cache-memory/) で整理しています。 一方、固定区画・可変区画・ページングなど「主記憶をどう割り当てて管理するか」は、[主記憶管理方式の違い](/fe/memory-management-methods/) で確認できます。
