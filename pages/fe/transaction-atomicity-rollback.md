...
description: "データベースのトランザクションを、原子性、COMMIT、ROLLBACK、デッドロックとの関係から初心者向けに整理します。ACID特性の見分け方と、異常終了後の表の状態を判断する手順をFE試験の選択肢の切り方とあわせて解説します。"
...
科目Aでは、ACID特性の説明文を読み、どの性質を表しているかを判断する問題が出ます。

まずは、次の4つのキーワードで切り分けます。
...
COMMIT後は障害が起きても失われない
→ D：Durability（永続性）
```

### ACIDを選択肢から見分ける判断表

| 説明文のポイント | 該当する性質 | 判断キーワード |
|---|---|---|
| トランザクション内の処理を、すべて実行するか、すべて取り消す | 原子性 | 全部か何もなしか・ROLLBACK |
| トランザクション前後でデータの矛盾を生じさせない | 一貫性 | 制約・整合性・矛盾しない |
| 複数のトランザクションが互いに不当に影響しない | 独立性 | 同時実行・排他制御・ロック |
| 正常終了後の更新結果を障害後も保持する | 永続性 | COMMIT後・障害後も残る |

### 原子性と永続性は「いつの話か」で切る

特に混同しやすいのが、原子性と永続性です。
...
つまり、**途中の失敗を見るなら原子性、COMMIT後の障害を見るなら永続性**と考えると判断しやすくなります。

### 「何度実行しても同じ結果」はACIDではない

「同じ処理を何度実行しても結果が同じ」という説明は、**べき等性（Idempotency）**です。
