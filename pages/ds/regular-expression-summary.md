...
description: "正規表現を、文字クラス、繰返し回数、行頭・行末アンカー、グループを組み合わせて文字列形式を検索・検証する道具として整理します。郵便番号・日付・メール形式の例から、部分一致と全文一致、`.`と文字としてのピリオド、量指定子の範囲を見分け、形式に一致しても実在性や日付の意味までは保証しない点を押さえます。"
