...
SG試験では、責任分担の表を見て、IaaS・PaaS・SaaSのどれに当たるかを選ばせることがあります。

PaaSでは、利用者は主にアプリケーションを開発・設定し、データや権限を管理します。

一方で、OS、DBMS、ハードウェアなどの基盤部分は、クラウド事業者側の管理に寄ります。

| 対象 | PaaSでの見方 |
|---|---|
| アプリケーション | 利用者が開発・設定・脆弱性対策を行う |
| データ | 利用者がアクセス制御や暗号化設定を行う |
| DBMS | 事業者側の管理に寄ることが多い |
| OS | 事業者側の管理に寄ることが多い |
| ハードウェア | 事業者側が物理的に管理する |

そのため、選択肢で「アプリケーションのセキュアプログラミングや脆弱性診断は利用者が行うが、OSやDBMSの修正プログラム適用は利用者責任ではない」と読める場合は、PaaSを考えます。

IaaSではOSやミドルウェアの管理まで利用者側に広がりやすく、SaaSではアプリケーション本体は事業者が提供します。
