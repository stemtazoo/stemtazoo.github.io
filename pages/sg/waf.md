...
### WAFは脆弱性そのものを修正する対策ではない

IPAの「Web Application Firewall（WAF）読本」では、WAFはWebアプリケーションの脆弱性を悪用した攻撃から保護する対策の一つとして説明されています。

ただし、WAFを導入しても**Webアプリケーションに存在する脆弱性そのものがなくなるわけではありません**。

本来は、安全な設計・実装や脆弱性の修正によって根本原因を取り除くことが基本です。WAFは、それらの対策を補い、攻撃による影響を低減するための防御策として使います。
...
- 脆弱性が見つかったが、すぐにアプリケーションを修正できない場合の補完策として使うとき

WAFは、アプリケーションの修正だけではすぐに対応できない場合にも、攻撃を遮断してリスクを下げるために利用できます。

一方で、WAFを導入したからといって脆弱性の修正が不要になるわけではありません。
...
- 「プログラムの動作範囲を制限する仕組み」  
- 「侵入者をおびき寄せる仕組み」
- 「WAFを導入すればWebアプリケーションの脆弱性そのものが修正される」

### ⭕ 正しい理解
- **通信内容を解析して攻撃を遮断する仕組み**
- **脆弱性の修正を補完し、攻撃による影響を低減する対策**
