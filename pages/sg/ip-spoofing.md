...
ア. 送信元IPアドレスを偽装し、別の端末からの通信のように見せる攻撃である。  
イ. IPアドレスを端末へ自動的に割り当てる仕組みである。  
ウ. IPアドレスとドメイン名の対応を正規に管理する仕組みである。  
エ. IPアドレスをMACアドレスへ対応付ける通常のプロトコルである。  
...
### 解説
- ア：送信元を偽る点がIPスプーフィングの本質です。
- イ：DHCPの説明です。
- ウ：DNSの説明です。
- エ：ARPの説明です。

👉 判断ポイント
IPスプーフィングは「送信元IPを偽る」、DHCP/DNS/ARPは通常のネットワーク機能。
