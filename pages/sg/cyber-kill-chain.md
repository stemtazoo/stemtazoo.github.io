...
A. 攻撃が成功した後の復旧手順だけを整理し、復旧時間を短くするために使う。  
B. 攻撃の流れを段階に分け、各段階でどのように防止・検知・抑止するかを考える。  
C. 脆弱性の深刻度を0.0〜10.0で数値化し、パッチ適用の優先順位を決める。  
D. 利用者の役職ごとにアクセス権限をまとめ、権限管理を効率化する。  
...
### 解説
- A：復旧だけに限定する考え方ではありません。攻撃の途中段階で止める視点が重要です。
- B：正解です。攻撃を段階に分け、どこで止めるかを考えます。
- C：CVSSの説明です。深刻度評価の指標であり、攻撃段階の整理ではありません。
- D：RBACの説明に近く、アクセス制御モデルの話です。
