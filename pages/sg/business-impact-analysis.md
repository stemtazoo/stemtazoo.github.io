...
description: ビジネスインパクト分析（BIA）を、災害や障害で業務が停止したときの時間経過による損失を評価し、重要業務と復旧優先順位を決めるBCP策定前の分析として整理します。許容できる最大停止時間MTDと目標復旧時間RTO、復旧時点を示すRPOを比較し、復旧手順の作成や訓練そのものとする誤答を切ります。
...
**RPO（Recovery Point Objective）** は、目標復旧時点です。

簡単にいうと、**どの時点までデータを戻せれば業務上許容できるか**を示す目標です。

RTOが「いつまでに再開するか（時間）」を表すのに対して、RPOは「どの時点のデータ状態まで戻すか（時点）」を表します。

たとえばRPOが「1時間」の場合、障害発生時点から見て最大1時間分のデータ損失までを許容する設計になります。

### MTBF・MTTR・RPO・RTOの切り分け

SG試験では、事業継続や復旧目標の問題で、MTBF・MTTR・RPO・RTOを並べて問われることがあります。

| 用語 | 見るもの | 判断ポイント |
|---|---|---|
| MTBF | 故障と故障の間の平均稼働時間 | 壊れにくさ・平均故障間隔 |
| MTTR | 障害発生から復旧完了までの平均時間 | 実際の復旧にかかった平均時間 |
| RPO | どの時点のデータまで戻すか | データ損失をどこまで許容するか |
| RTO | いつまでに業務・サービスを復旧させるか | 業務再開までの目標時間 |

選択肢で **「何時間以内に業務やサービスを再開するか」** という復旧期限の話が出たら、RTOを疑います。

一方、**「どの時点のデータまで戻すか」** ならRPO、**「平均復旧時間」** ならMTTR、**「平均故障間隔」** ならMTBFです。
