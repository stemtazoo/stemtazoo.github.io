---
layout: page
title: "ハイブリッド暗号方式とは？速度と安全性を両立する仕組み【SG試験】"
description: "ハイブリッド暗号方式は公開鍵暗号と共通鍵暗号を組み合わせる方式です。鍵配布の安全性と高速処理を両立する仕組みを、SG試験・情報セキュリティマネジメント試験での判断ポイントとともに整理します。"
...
A. 共通鍵暗号だけで鍵配送と本文暗号化を行う方式である。  
B. 公開鍵暗号で本文をすべて暗号化し、共通鍵暗号は使わない方式である。  
C. 公開鍵暗号で共通鍵を安全に共有し、本文は共通鍵暗号で高速に暗号化する方式である。  
D. ハッシュ関数で鍵を生成し続けることで鍵交換を不要にする方式である。  
...
### 解説
- A：鍵配送の課題が解決できません。  
- B：公開鍵暗号のみだと処理負荷が高くなりやすいです。  
- C：鍵配送の安全性と処理速度を両立する典型構成です。  
- D：ハッシュは完全な鍵交換の代替にはなりません。  
