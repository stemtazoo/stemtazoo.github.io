...
A. すべての業種に共通する労務管理の法令要件である。  
B. クレジットカード会員データを扱う事業者向けのセキュリティ基準である。  
C. 無線LAN暗号化方式の技術仕様そのものである。  
D. インシデント報告の書式だけを定めたガイドである。  
...
- A：労務法令ではありません。  
- B：カード情報保護のための業界基準です。  
- C：暗号方式そのものを定義する規格ではありません。  
- D：報告書式限定ではなく管理策全般を扱います。  
