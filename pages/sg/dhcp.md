...
description: "DHCPはIPアドレスを自動で割り当てる仕組みを理解するための用語です。この記事では仕組み・役割・使いどころを押さえ、SG試験・情報セキュリティマネジメント試験とで狙われるひっかけポイントを解説します。"
permalink: /sg/dhcp/
...
ア. ドメイン名から対応するIPアドレスを調べる。  
イ. 端末にIPアドレスなどのネットワーク設定を自動的に割り当てる。  
ウ. 異なるネットワーク間でIPアドレスを変換する。  
エ. IPアドレスからMACアドレスを調べて対応付ける。  
...
### 解説
- ア：これはDNSの役割です。
- イ：DHCPは端末にIPアドレスなどを自動配布する仕組みです。
- ウ：これはNATの役割です。
- エ：これはARPの役割です。
