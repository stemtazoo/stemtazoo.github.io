...
description: "主キーは表の行を一意に識別する項目、外部キーは別の表との関係を表す項目です。この記事では主キーと外部キーの違い、ER図やデータモデルとの関係、SG試験・情報セキュリティマネジメント試験で迷いやすいポイントを整理します。"
...
A. 主キーは表内で行を一意に識別し、外部キーは他表との関連を保つ。
B. 主キーは重複を許可し、外部キーは必ず重複不可である。
C. 主キーは文字列型では定義できない。
D. 外部キーは同一表の参照では使えない。
