...
WPA2の暗号化方式を整理するときは、代表的な方式として **AES系の暗号化** を押さえます。

| 問われ方 | WPA2との関係 | 試験での切り方 |
|---|---|---|
| 共通鍵暗号・AES系の方式 | WPA2で利用される代表的な暗号化方式 | 正解候補 |
| 古い無線LAN方式で使われた弱いストリーム暗号 | WPA2の改善前の方式として切り分ける | 不正解候補 |
| 公開鍵暗号方式 | 鍵交換や署名の文脈で使われることが多い | 無線通信そのものの暗号化方式としては切る |
| 楕円曲線暗号 | 公開鍵暗号の一種として扱う | WPA2の暗号化アルゴリズムとしては切る |

WPA2は、WEPや古いWPAで使われたRC4系の弱点を改善し、AES系の暗号化を使う点が重要です。

問題文が無線通信の暗号化方式を尋ねているなら、認証方式や公開鍵暗号の説明ではなく、通信内容を暗号化する方式に注目します。

## まとめ（試験直前用）

- 無線LAN対策は、暗号化・接続制限・分離を組み合わせて考える
- WPA2 / WPA3 は、無線通信の内容を守る対策
- WPA2の暗号化方式では、AES系の暗号化を押さえる
