...
description: BECは、取引先や経営者を装ったメールで送金や情報提供をだますビジネスメール詐欺です。メール配送妨害、中継悪用、信用毀損目的のなりすましとの違いをSG試験向けに整理します。
...
### 誤解4：組織をメールで困らせる攻撃はすべてBECである

BECは、業務メールを悪用して**送金・支払・振込先変更などの金銭被害**につなげる点が中心です。

メールを使う攻撃でも、目的が違えば別の用語として切り分けます。

| 選択肢の内容 | 判断 |
|---|---|
| 取引先や経営者になりすまし、偽の請求や送金をさせる | BECを疑う |
| メールを大量に送り付けて配送や利用を妨げる | メールサービスへの妨害が中心 |
| 中継設定の甘いメールサーバを迷惑メール配送に使う | メールサーバ運用・中継悪用の話 |
| 組織名をかたって評判を落とす内容を送る | 信用毀損目的のなりすましが中心 |

👉 判断ポイント：
**業務上の支払・送金判断をだますならBEC、メール基盤の妨害・中継悪用・評判低下が中心なら別論点**として切ります。
...
- SG試験では「ウイルス対策だけで十分」「緊急なので確認省略」「支払判断をだます話ではないメール妨害」を切る
