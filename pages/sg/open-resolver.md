...
JPCERT/CCの[オープンリゾルバー確認サイト](https://www.jpcert.or.jp/magazine/security/openresolver.html)でも、オープンリゾルバーは外部の不特定のIPアドレスから再帰的な問い合わせを許可しているDNSサーバーとして説明されています。
...
#### SG試験での選択肢の切り方：DNS amp対策

DNS amp攻撃の踏み台化を防ぐ問題では、**再帰問い合わせを使える相手を、社内や契約者など必要な範囲に限定しているか**を見ます。

正解候補になりやすいのは、次の方向性です。

- 社外の不特定多数から再帰問い合わせを利用されないようにする
- 問い合わせ元のネットワーク範囲を制限する
- 公開するDNS機能と、内部利用者向けの名前解決機能を同じ考えで扱わない
- DNSサーバが不要な問い合わせを受け続けないように運用する

一方で、次のような説明は、DNS ampの踏み台化防止とは論点がずれます。

| 説明の方向性 | 切り分け |
|---|---|
| ドメイン登録者の確認 | 登録情報の確認であり、再帰問い合わせの悪用防止ではない |
| 複数サーバへの振分け | 可用性や負荷分散の話であり、踏み台防止そのものではない |
| DNS応答の真正性確認 | DNSSECなどの話であり、オープンリゾルバ対策とは別論点 |
| 誰でも再帰問い合わせを使える設定 | 踏み台化の原因になり得る |

SG試験では、DNS amp対策を「DNSの詳しい設定名」ではなく、**不特定多数に再帰問い合わせを使わせない設計かどうか**で判断すると切りやすくなります。
...
- ア. DNS応答にデジタル署名を付け、リソースレコードの正当性と完全性を検証する仕組みである。
- イ. 不特定多数からの再帰問い合わせを受け付けるDNSキャッシュサーバであり、DNSリフレクター攻撃の踏み台として悪用されるおそれがある。
- ウ. プライマリDNSサーバとセカンダリDNSサーバを二重化し、名前解決の可用性を高める仕組みである。
- エ. DNSキャッシュに偽の情報を覚え込ませ、利用者を偽サイトへ誘導する攻撃である。
