...
description: "パケットフィルタリングは通信データを条件で選別し、許可・拒否を行う仕組みです。ファイアウォールの基本機能としての役割や、SG試験・情報セキュリティマネジメント試験でのブラックリスト・ホワイトリストの判断ポイントを整理します。"
...
A. アプリの業務ロジックまで解析して不正を判定する。  
B. IPアドレスやポート番号などの条件で通信を許可/遮断する。  
C. ユーザ教育計画を自動生成する仕組みである。  
D. 端末のバックアップ運用を代替できる。  
