...
ア. 同じ入力でも毎回異なる固定長出力になる。  
イ. 可逆変換なので、ハッシュ値から元データを復元できる。  
ウ. 任意長入力を固定長に要約し、改ざん検知に利用される。  
エ. 鍵を使ってのみ計算できるため、公開検証には向かない。  
...
### 解説
- ア：同一入力なら通常は同一出力（決定的）です。  
- イ：ハッシュは原則として不可逆です。  
- ウ：改ざん検知や整合性確認で使う基本性質です。  
- エ：一般的なハッシュは鍵不要です（鍵付きはMAC系）。  
