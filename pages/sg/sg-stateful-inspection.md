...
**ステートフルインスペクションとは、ファイアウォールが通信セッションの状態を記録し、その通信の続きとして正しいパケットだけを通す方式です。**

「ステートフル」は、通信の状態を覚えているという意味です。社内PCが開始した通信を記録しておき、その応答として戻ってきたパケットなら許可します。

ただし、HTTPリクエストの本文などを詳しく検査する方式ではありません。Webアプリへの攻撃を検査する役割は、主にWAFが担います。
...
迷ったら、**「パケット単体を見るのか、通信の流れを見るのか、Web通信の内容を見るのか」**を確認します。
...
- ステートフルインスペクションは、**通信セッションの状態を記録するファイアウォール方式**
- **通信の流れ・状態・戻り通信**がキーワード
- パケット単体を見るのはパケットフィルタリング、Web通信の内容を見るのはWAF
