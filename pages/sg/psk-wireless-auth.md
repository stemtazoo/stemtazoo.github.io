...
A. 利用者ごとに証明書を発行し、共通鍵は使わない方式である。  
B. 端末とAPで事前共有鍵を使って認証する方式で、小規模環境でよく使われる。  
C. 認証サーバ（RADIUS）が必須で、鍵共有は不要である。  
D. 一度接続した端末は鍵なしで永続的に接続できる。  
...
### 解説
- A：それは証明書ベース（802.1X/EAP系）に近いです。  
- B：PSKの基本的な特徴です。  
- C：PSKはRADIUS必須ではありません。  
- D：鍵による認証は継続して必要です。  
